...
plot_molecule(fe_tri, label_mode = "none", label = "element", label_colour = NULL, clear_scene = FALSE)
rgl::next3d()
# plot_molecule(pd, label_mode = "none", label = "element", label_colour = NULL, clear_scene = FALSE)
plot_molecule(fe_tri, label_mode = "no_atoms", label = "element", label_colour = NULL, clear_scene = FALSE)
rglwidget()
```
...
# fe_tri@atoms |>
#   dplyr::filter(elena == "N5")


# Locate the centre of our bonding atom positions
bonding_atom_center <- fe_tri |>
  structures::filter_atoms(eleno = bonding_atom_numbers) |>
  structures::locate_molecule_center()

# fe_tri |>
#   structures::set_anchor_by_position(bonding_atom_center) |>
#   plot_molecule(show_anchor = TRUE); rglwidget()

# Translate atom so bonding atom is centered
fe_tri_centered <- fe_tri |>
  structures::set_anchor_by_position(bonding_atom_center) |>
  structures::translate_molecule_to_origin()

fe_tri_centered |>
  structures::filter_atoms(bonding_atom_numbers) |>
  as.data.frame() |>
  dplyr::pull(z) |>
  mean()
# df_tri_atom <- dplyr::as_tibble(fe_tri@atoms)
# df_tri_bonds <- dplyr::as_tibble(fe_tri@bonds)

# Special atom
testelement = "Fe1"
testelementno = 1

# Create segments to translated center
segments_to_origin <- fe_tri_centered@atoms |>
  ...
  move::convert_tidy_segments_to_interleaved()

# Create segments from testelement to origin
segments_to_origin_special <- fe_tri_centered@atoms |>
...
with(segments_to_anchor, rgl::segments3d(x, y, z, lit=FALSE, color = "green"))
with(segments_to_origin_special, rgl::segments3d(x, y, z, lit=FALSE, color = "orange"))
...
# Get Iron Position
specialpos = fe_tri_centered |>
  structures::fetch_atom_position(testelementno)

# Get axis and angle of rotation that would get our iron to face the top right corner
params <- move::rotate_vector_to_align_with_target(v = specialpos, target = target, return = "axis_plus_angle")

# Apply that rotation to all the atoms
fe_tri_c3aligned <- fe_tri_centered |>
  structures::transform_molecule(move::rotate_vector_around_axis, rotation_axis = params$axis, angle = params$angle)
...
segments_to_center_c3aligned <- fe_tri_c3aligned@atoms |>
   dplyr::filter(eleno %in% bonding_atom_numbers) |>
   dplyr::mutate(
     xend = 0,
     yend = 0,
...
segments_from_special_to_center_c3aligned <- fe_tri_c3aligned@atoms |>
  dplyr::filter(eleno == testelementno) |>
  dplyr::mutate(xend = 0, yend = 0, zend=0) |>
  trans::convert_tidy_segments_to_interleaved()
...
points3d(cube_face, col="red")
spheres3d(cube_corners, color = "pink", radius=0.1)
with(segments_to_center_c3aligned, rgl::segments3d(x, y, z, lit=FALSE, color = "green"))
with(segments_from_special_to_center_c3aligned, rgl::segments3d(x, y, z, lit=FALSE, color = "orange"))
...
# Pick one of the n5 atoms (bonding atoms
target_binding_atom_no <- bonding_atom_numbers[1]

v <- fetch_atom_position(fe_tri_c3aligned, target_binding_atom_no)

# Find paramaters to rotate one of the binding atoms into plane_normal
params_c34 <- move::rotate_vector_into_a_plane(v, c34_plane, return = "axis_plus_angle")

params_c34

# Apply that rotation to all the atoms
fe_tri_c34aligned <- fe_tri_c3aligned |>
  structures::transform_molecule(move::rotate_vector_around_axis, rotation_axis = params_c34$axis, angle = params_c34$angle)


# Compute segment between special testing atom (iron) and the zero position. Line should fall along the c4 axis.
segments_to_center_c34 <- fe_tri_c34aligned@atoms |>
  dplyr::filter(eleno %in% bonding_atom_numbers) |>
  dplyr::mutate(xend = 0, yend = 0, zend=0) |>
  trans::convert_tidy_segments_to_interleaved()
...
points3d(cube_face, col="red")
with(segments_to_center_c34, rgl::segments3d(x, y, z, lit=FALSE, color = "green"))
spheres3d(cube_corners, color = "pink", radius=0.1)
c45_3d()
rglwidget()
```
...
vertex_coords_raw <- tetrahedron$vb[1:3, ]
vertex_coords <- t(vertex_coords_raw) 
rownames(vertex_coords) <- LETTERS[seq_along(vertex_coords[,1])]
...
face_centroids <- t(sapply(faces, function(idx) colMeans(vertex_coords[idx, ])))
...
edge_centers <- t(sapply(edges, function(e)
  colMeans(vertex_coords[e, , drop = FALSE])
))


# Compute C3 Vector & C2 vector
c3_tetrahedron <- move::create_vector_from_start_end(start = vertex_coords["A",], end = face_centroids["BCD",])
c2_tetrahedron <- move::create_vector_from_start_end(start = edge_centers["AB",], end = edge_centers["CD",])


# Generate Visualisation with
# rgl::wire3d(tetrahedron, lit = FALSE, color="red")
# rgl::spheres3d(x=vertex_coords, radius = 0.1, color = "pink", lit=FALSE)
# rgl::points3d(x=face_centroids, size = 10, color = "orange", lit=FALSE)
# rgl::points3d(x=edge_centers, size = 10, color = "white", lit=FALSE)
# rgl::spheres3d(x=c(0, 0, 0), radius = 0.1, color = "white", lit=FALSE)
# rgl::abclines3d(x=face_centroids["BCD",], a=c3_tetrahedron, color ="gold", lit=FALSE)
# rgl::abclines3d(x=edge_centers["AB",], a=c2_tetrahedron, color ="pink", lit=FALSE)
